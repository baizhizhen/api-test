...
<template>
    <div class="message-main-con">
        <div class="message-mainlist-con">
            <div>
                <i-button @click="setCurrentMesType('unread')"
                          size="large"
                          type="text"
                          long>
                    <transition name="mes-current-type-btn">
                        <i-icon v-show="currentMessageType === 'unread'"
                                type="checkmark">
                        </i-icon>
                    </transition>
                    <span class="mes-type-btn-text">未读消息</span>
                    <i-badge class="message-count-badge-outer"
            ...
            <div>
                <i-button @click="setCurrentMesType('hasread')"
                          size="large"
                          type="text"
                          long>
                    <transition name="mes-current-type-btn">
                        <Icon v-show="currentMessageType === 'hasread'"
                              type="checkmark">
                        </Icon>
                    </transition>
            ...
            <div>
                <i-button @click="setCurrentMesType('recyclebin')"
                          size="large"
                          type="text"
                          long>
                    <transition name="mes-current-type-btn">
                        <i-icon v-show="currentMessageType === 'recyclebin'"
                                type="checkmark">
                        </i-icon>
                    </transition>
        ...
        <div class="message-content-con">
            <transition name="view-message">
                <div v-if="showMesTitleList"
                     class="message-title-list-con">
                    <i-table ref="messageList"
                    ...
                    <div class="message-content-top-bar">
                        <span class="mes-back-btn-con">
                            <i-button type="text" @click="backMesTitleList">
                                <i-icon type="chevron-left"></i-icon>&nbsp;&nbsp;返回
                            </i-button>
                        </span>
                        <h3 class="mes-title">{{ mes.title }}</h3>
                    </div>
                    <p class="mes-time-con">
                        <i-icon type="android-time"></i-icon>&nbsp;&nbsp;{{ mes.time }}
                    </p>
